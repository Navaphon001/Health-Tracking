...
class AboutYourselfService {
  ...
  // Create about yourself info
  static Future<Map<String, dynamic>?> createAboutYourself({
    required String id,
    ...
    try {
      ...
      print('Sending request to: $baseUrl/about_yourself');
      print('Request body: ${jsonEncode(body)}');
      
      final response = await http.post(
        Uri.parse('$baseUrl/about_yourself'),
        headers: {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        snackFn?.call('บันทึกข้อมูลเกี่ยวกับตัวคุณเรียบร้อยแล้ว');
        return data;
      } else {
  ...
  // Update about yourself info
  static Future<Map<String, dynamic>?> updateAboutYourself({
    required String aboutId,
    String? healthDescription,
    String? healthGoal,
    SnackFn? snackFn,
  }) async {
    try {
      ...
      final response = await http.put(
        Uri.parse('$baseUrl/about_yourself/$aboutId'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
      ...
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        snackFn?.call('อัปเดตข้อมูลเกี่ยวกับตัวคุณเรียบร้อยแล้ว');
        return data;
      } else {
        final errorData = jsonDecode(response.body);
  ...
  // Get about yourself info for current user
  static Future<List<Map<String, dynamic>>?> getAboutYourselfList({
    SnackFn? snackFn,
  }) async {
    try {
      final token = await _getToken();
  ...
  // Get specific about yourself info by ID
  static Future<Map<String, dynamic>?> getAboutYourself({
    required String aboutId,
    SnackFn? snackFn,
  }) async {
    try {
  ...
  // Delete about yourself info
  static Future<bool> deleteAboutYourself({
    required String aboutId,
    SnackFn? snackFn,
  }) async {
    try {
      ...
      if (response.statusCode == 200) {
        snackFn?.call('ลบข้อมูลเกี่ยวกับตัวคุณเรียบร้อยแล้ว');
        return true;
      } else {
        final errorData = jsonDecode(response.body);
