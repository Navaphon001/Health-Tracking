...
import '../services/user_profile_service.dart'; // สำคัญ
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void initState() {
    super.initState();
    // ตั้ง SnackBar callback หลังเฟรมแรก เพื่อเลี่ยงบริบทที่ยังไม่พร้อม
  ...
  Future<void> _doLogin() async {
    ...
    // ✅ กัน null: currentState อาจยังไม่พร้อม
    if (!(_formKey.currentState?.validate() ?? false)) return;

    final ok = await auth.login(_email.text.trim(), _password.text.trim());
    if (!ok) return;

    // ✅ ตัดสินใจปลายทางจากสถานะโปรไฟล์ใน SQLite
    const userId = 'local'; // still mock
    final status = await UserProfileService.instance.getStepStatus(userId);
    // { step1:bool, step2:bool, step3:bool, completed:bool }

    String nextRoute;
    if (status['completed'] == true) {
      nextRoute = '/main';
    } else if (status['step1'] != true) {
      nextRoute = '/profile-setup-step1';
    } else if (status['step2'] != true) {
      nextRoute = '/profile-setup-step2';
    } else {
      nextRoute = '/profile-setup-step3';
    }
